...
machine with 8 or fewer cores it silently drops to a single worker.
...
| `uv run python -m src.utils.hpxml.gen_hpxml_column_list [--samples 400] [--batches 5]` | Regenerate the exported column contract |
...
settings, and roughly 27 seconds of wall clock each on a 12-core host.
